...
## Repository implementation (Next.js / Tailwind)

This codebase maps the tokens above to `src/app/globals.css` (`:root` + `@theme inline`) and Tailwind semantic classes. **Do not duplicate hex values in components** — extend CSS if a new token is needed.

| DESIGN.md | CSS variable | Tailwind (examples) |
|-----------|----------------|---------------------|
| `{colors.primary}` | `--primary` | `bg-tint`, `text-tint` (`--tint` aliases `--primary`) |
| `{colors.primary-focus}` | `--primary-focus` | `:focus-visible` outline; `ring-primary-focus` |
| `{colors.primary-on-dark}` | `--primary-on-dark` | reserved for future dark tiles / `text-primary-on-dark` |
| `{colors.canvas}` | `--canvas` | `bg-surface` (surface = canvas) |
| `{colors.canvas-parchment}` | `--canvas-parchment` | `bg-background`, `bg-canvas-parchment` |
| `{colors.surface-pearl}` | `--surface-pearl` | Pearl / secondary fills |
| `{colors.ink}` | `--ink` | `text-foreground` |
| `{colors.hairline}` | `--hairline` | `border-hairline` |
| `{colors.divider-soft}` | `--divider-soft` | `border-divider-soft` (e.g. Pearl button ring) |
| `{colors.ink-muted-80}` | `--ink-muted-80` | `text-ink-muted-80` |
| Body 17px / 1.47 / tracking | `body { font-size: 17px; … }` | base paragraph rhythm |

**Buttons:** Primary pills use `bg-tint` + `motion-safe:active:scale-[0.98]`. Secondary pearl per `button-pearl-capsule`: `rounded-[11px] border-[3px] border-divider-soft bg-surface-pearl text-ink-muted-80`.

**Cards:** Utility cards use `rounded-[18px] border border-hairline` — **no** drop-shadow on cards (per Do’s and Don’ts).

`themeColor` in `layout.tsx` is `#0066cc` to match Action Blue.

**Conversion / B2B copy:** zentrale sachliche Texte und Stützen liegen in `src/lib/institutional-copy.ts` (Startseiten-Säulen, Shop-Bullets, Pre-Footer-CTA, Anfrage-Kurzpunkte). Anpassen, sobald reale SLAs oder rechtliche Texte final sind.
